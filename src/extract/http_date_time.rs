        ...
        match ctx.callsite().source() {
            Header(header_name) => {
                let value = match ctx.request().headers().get(header_name) {
                    Some(value) => value,
                    None => return Immediate::err(Error::missing_argument()),
                };

                let value = match value.to_str() {
                    Ok(s) => s,
                    Err(_) => return Immediate::err(Error::invalid_argument(&"invalid UTF-8 string")),
                };

                match DateTime::parse_from_rfc2822(&value) {
                    Ok(dt) => Immediate::ok(HttpDateTime::normalize(dt.with_timezone(&Utc))),
                    Err(e) => Immediate::err(Error::invalid_argument(&e)),
                }
            }
